...
} from '/var/task/Downloads/dramabox-main/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/Downloads/dramabox-main/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/Downloads/dramabox-main')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/Downloads/dramabox-main',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/Downloads/dramabox-main/.netlify/dist/run/handlers/server.js')
